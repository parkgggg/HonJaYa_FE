...
export const postData = async (endpoint: any, data: any, dest: any) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to post data ${endpoint}: ${response.statusText}`
      );
    }
    if(!response.headers.get('content-type') || !response.headers.get('content-type')?.includes("application/json")) return await response.text();
    // try {
    //   return await response.json();
    // } catch (jsonError) {
    //   console.error(
    ...
    console.error(`Failed to post data to asdfasdf${endpoint}:`, error);
    throw error;
  }
};
