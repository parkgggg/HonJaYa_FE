...
const ToggleSwitch = ({withTeam, setWithTeam}: Props) => {
    ...
    return (
        <div
            className={`mx-2 relative inline-flex items-center h-6 rounded-full w-11 transition-colors duration-200 ease-in-out cursor-pointer ${withTeam ? 'bg-main-color' : 'bg-gray-300'}`}
            onClick={setWithTeam}
        >
